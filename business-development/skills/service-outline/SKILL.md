...
argument-hint: "[service-name] [--type=<type>] [--new|--update <source>] [--brief] [--output=<path>] [--refresh]"
---
...
4. **Ingest the source (update mode only).** For `--update <source>`, read the source with the matching adapter (see `references/input-adapters.md`): `Read` for an uploaded file or local path, the Notion MCP `notion-fetch` for a Notion URL, `WebFetch` for a web URL. Map its content onto the 10 modules and note which sections are present, thin, or outdated.
5. **Interview the user (mandatory gate — never write before this).** Using `AskUserQuestion`, ask a focused, batched set of questions from `references/interview-guide.md`, pre-filled from the profile and any ingested source. Ask only what is missing. Always confirm the two addendum triggers:
